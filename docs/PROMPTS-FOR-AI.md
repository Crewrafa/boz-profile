...
```
I'm working on the ERP Platform, a staffing SaaS. Stack: React+Vite on Vercel, Supabase backend, Anthropic Claude for AI. 6 modules: Client (8-step form), Recruiter (review), Ana (soft skills), Admin (full ATS+kanban), Sales (3 tabs), Finance (per-profile calculator).
