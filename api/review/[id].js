...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    if (!assignment_id || !uuidRe.test(assignment_id)) return res.status(400).json({ error: "Invalid assignment_id" });
    if (!["accepted", "rejected", "pending"].includes(decision)) return res.status(400).json({ error: "Invalid decision" });
    // Verify review is still authorized before accepting decisions
    const pR = await fetch(`${url}/rest/v1/profiles?id=eq.${id}&select=profile_data,deleted_at`, { headers: H });
    const pD = (await pR.json())[0];
    if (!pD || pD.deleted_at || !pD.profile_data?.reviewAuthorized) return res.status(403).json({ error: "Review not authorized" });
  ...
  try {
    ...
    if (!profile || profile.deleted_at) return res.status(404).send("Profile not found");
    const pd = profile.profile_data || {};

    // Check if review is authorized
    if (!pd.reviewAuthorized) {
      res.setHeader("Content-Type", "text/html; charset=utf-8");
      return res.send(`<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>ERP Platform — Not Ready</title><link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@600;700;800&family=DM+Sans:wght@400;500&display=swap" rel="stylesheet"><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'DM Sans',sans-serif;background:#f1f5f9;display:flex;align-items:center;justify-content:center;min-height:100vh;padding:20px}</style></head><body><div style="text-align:center;max-width:420px"><div style="font-size:28px;font-weight:800;color:#0D2550;font-family:'Plus Jakarta Sans',sans-serif;letter-spacing:1.5px;margin-bottom:8px">ERP<span style="color:#22d3ee">.</span></div><div style="font-size:10px;color:#94a3b8;letter-spacing:3px;text-transform:uppercase;margin-bottom:32px">Platform</div><div style="font-size:48px;margin-bottom:16px">🔒</div><div style="font-size:20px;font-weight:700;color:#0D2550;font-family:'Plus Jakarta Sans',sans-serif">Candidates Not Ready Yet</div><div style="font-size:14px;color:#64748b;margin-top:8px;line-height:1.6">Our team is still working on finding the best candidates for this position. You'll receive a notification when they're ready for your review.</div><div style="margin-top:24px;font-size:12px;color:#94a3b8">ERP Platform · Confidential</div></div></body></html>`);
    }
...
function buildReviewHTML(profile, assignments, docs, pd, e, profileId) {
  ...
  const cards = assignments.map((a, idx) => {
    ...
    return `<div class="card" id="card-${a.id}" data-decision="${decision}">
      ...
      <div class="docs-row">
        ${cvDoc?.signed_url ? `<a href="${cvDoc.signed_url}" target="_blank" rel="noopener" class="doc-btn">📄 CV</a>` : `<span class="doc-pending">📄 CV pending</span>`}
        ${softDoc?.signed_url ? `<a href="${softDoc.signed_url}" target="_blank" rel="noopener" class="doc-btn">🧠 Soft Skills</a>` : `<span class="doc-pending">🧠 Soft eval pending</span>`}
        ${hardDoc?.signed_url ? `<a href="${hardDoc.signed_url}" target="_blank" rel="noopener" class="doc-btn">⚙️ Hard Skills</a>` : `<span class="doc-pending">⚙️ Hard eval pending</span>`}
      </div>
...
<title>${clientCompany || "ERP Platform"} — Candidate Review</title>
...
<div class="top-bar">
  <div><div class="logo">ERP<span>.</span></div></div>
...
<div class="ft"><strong>ERP Platform</strong> · Confidential · ${dateStr}</div>
