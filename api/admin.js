// ERP Platform Admin API — profiles, candidates, assignments
// Accessible by: admin, recruiter, sales, finance roles (per-endpoint gating below)
// Security: rate limiting, input sanitization, auth verification

const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
const PROFILE_STATUSES = ["new", "pending_review", "pending_soft", "in_progress", "sourcing", "filled", "closed"];
...
const ALLOWED_ROLES = ["admin", "recruiter", "sales", "finance"];
...
async function verifyAccess(req, url, key) {
  ...
  const devMap = {
    "dev": { email: "psicologorafaelbaez@gmail.com", role: "admin" },
    "dev-recruiter": { email: "recruiter@erp-platform.local", role: "recruiter" },
    "dev-sales": { email: "sales@erp-platform.local", role: "sales" },
    "dev-finance": { email: "finance@erp-platform.local", role: "finance" },
  };
  if (devMap[token]) return devMap[token];
  try {
    ...
    if (!roles.length || !ALLOWED_ROLES.includes(roles[0].role)) return null;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "PATCH" && !action) {
      ...
      if (!id || !UUID_RE.test(id) || !status) return res.status(400).json({ error: "Valid id and status required" });
      if (!PROFILE_STATUSES.includes(status)) return res.status(400).json({ error: "Invalid status" });
    ...
    if (action === "restore") {
      ...
      if (!id || !UUID_RE.test(id) || !table_name) return res.status(400).json({ error: "Valid id and table_name required" });
      if (admin.role !== "admin") return res.status(403).json({ error: "Admin only" });
    ...
    if (action === "assign_candidate") {
      ...
      if (!UUID_RE.test(profile_id) || !UUID_RE.test(candidate_id)) return res.status(400).json({ error: "Invalid ID format" });
      const ms = Math.max(0, Math.min(100, parseInt(match_score, 10) || 0));
      const r = await fetch(`${url}/rest/v1/profile_candidates`, { method: "POST", headers: { ...H, "Prefer": "return=representation" },
        body: JSON.stringify({ profile_id, candidate_id, match_score: ms, notes: notes || "", status: "data_verification" }) });
      if (!r.ok) { const err = await r.text(); return res.status(r.status).json({ error: err }); }
      const d = await r.json();
      return res.status(200).json(d[0] || d);
    }
    ...
    if (action === "update_assignment") {
      ...
      const updates = {};
      if (status) {
        if (!CAND_STAGES.includes(status)) return res.status(400).json({ error: "Invalid stage" });
        updates.status = status;
      }
      if (match_score !== undefined) updates.match_score = Math.max(0, Math.min(100, parseInt(match_score, 10) || 0));
    ...
    if (action === "update_candidate") {
      ...
      if (!id || !UUID_RE.test(id) || !data) return res.status(400).json({ error: "Valid id and data required" });
    ...
    if (action === "upload_document") {
      ...
      if (!UUID_RE.test(candidate_id)) return res.status(400).json({ error: "Invalid candidate_id" });
      if (profile_id && !UUID_RE.test(profile_id)) return res.status(400).json({ error: "Invalid profile_id" });
    ...
    if (action === "update_client_decision") {
      ...
      if (!assignment_id || !UUID_RE.test(assignment_id)) return res.status(400).json({ error: "Invalid assignment_id" });
      ...
      if (!["admin", "sales"].includes(admin.role)) return res.status(403).json({ error: "Admin or Sales only" });
    ...
    if (action === "get_pricing") {
      if (!["admin", "finance"].includes(admin.role)) return res.status(403).json({ error: "Admin/Finance only" });
      const { profile_id } = req.body;
      if (profile_id) {
        if (!UUID_RE.test(profile_id)) return res.status(400).json({ error: "Invalid profile_id" });
