// ERP Platform Recruiter API — profile review, accept/reject, questions for Ana
...
async function verifyRecruiter(req, url, key) {
  ...
  const devMap = { "dev": { email: "psicologorafaelbaez@gmail.com", role: "admin" }, "dev-recruiter": { email: "recruiter@erp-platform.local", role: "recruiter" } };
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === "list_profiles") {
      const r = await fetch(
        `${url}/rest/v1/profiles?deleted_at=is.null&order=created_at.desc&select=id,created_at,client_name,client_company,client_email,role,category,seniority,experience,headcount,status,start_date,profile_data`,
        { headers: H }
      );
