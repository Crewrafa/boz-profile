...
// ═══════════ PDF (no KPIs, no desirable, no success, includes notes) ═══════════
  ...
  return(<div style={{position:"fixed",inset:0,zIndex:200,background:"rgba(15,23,42,0.6)",backdropFilter:"blur(4px)",display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={onClose}>
    <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:DS.radius.xxl,maxWidth:640,width:"100%",maxHeight:"85vh",overflow:"hidden",animation:"fadeUp .3s both",display:"flex",flexDirection:"column",boxShadow:DS.shadow.lg}}>
      ...
      <div style={{padding:"16px 28px",background:DS.surface.page,textAlign:"center",flexShrink:0}}>
        <div style={{fontSize:11,color:DS.text.faint,fontFamily:DS.font.body}}>© 2024 ERP Platform</div>
      </div>
    </div>
  </div>);
}
...
function Footer(){
  ...
  <div style={{maxWidth:1100,margin:"0 auto",position:"relative"}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"start",flexWrap:"wrap",gap:mob?16:24,marginBottom:mob?16:28}}>
      <div><div style={{fontSize:mob?20:24,fontWeight:800,color:"#fff",letterSpacing:2,fontFamily:DS.font.heading}}>E<span style={{color:"#4F6AFF"}}>R</span>P<span style={{color:DS.brand.cyan600}}>.</span></div><div style={{fontSize:9.5,color:"rgba(255,255,255,0.3)",letterSpacing:3.5,textTransform:"uppercase",marginTop:5,fontFamily:DS.font.body}}>ERP Platform</div></div>
    ...
    <div style={{display:"flex",flexDirection:mob?"column":"row",justifyContent:"space-between",alignItems:mob?"flex-start":"center",gap:8}}>
      <div style={{display:"flex",alignItems:"center",gap:10}}>
        <div style={{fontSize:10,color:"rgba(255,255,255,0.22)",fontFamily:DS.font.body}}>© 2024 ERP Platform. All Rights Reserved.</div>
...
// ═══════════ ROLE SELECTOR (no login — pick a role and enter) ═══════════
const ROLE_TOKENS=[
  {token:"dev",label:"Admin",icon:"⚡",desc:"Full ATS, kanban, pipeline, users, finance overview",color:"#1B6FE8",bg:"rgba(27,111,232,0.12)",border:"rgba(27,111,232,0.3)"},
  {token:"dev-sales",label:"Sales",icon:"💼",desc:"Invite clients, dashboards, deliver review links",color:"#10b981",bg:"rgba(5,150,105,0.12)",border:"rgba(5,150,105,0.3)"},
  {token:"dev-recruiter",label:"Recruiter",icon:"🔍",desc:"Review profiles, add notes & questions for Ana",color:"#fb923c",bg:"rgba(249,115,22,0.12)",border:"rgba(249,115,22,0.3)"},
  {token:"dev-ana",label:"Talent Discovery",icon:"🧠",desc:"Soft skills evaluation from client meetings",color:"#a78bfa",bg:"rgba(124,58,237,0.15)",border:"rgba(124,58,237,0.3)"},
  {token:"dev-finance",label:"Finance",icon:"💰",desc:"Revenue calculator & viability per profile",color:"#38bdf8",bg:"rgba(3,105,161,0.12)",border:"rgba(3,105,161,0.3)"},
  {token:"dev-client",label:"Client",icon:"👤",desc:"8-step form to define your ideal tech talent",color:"#cbd5e1",bg:"rgba(255,255,255,0.06)",border:"rgba(255,255,255,0.15)"},
];
function Login(){
  const pick=(t)=>{localStorage.setItem("sb-access-token",t);window.location.reload()};
  const w=useW();const mob=w<640;
  return(<div style={{minHeight:"100vh",background:"#060F22",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:DS.font.body,padding:16,position:"relative",overflow:"hidden"}}>
    ...
    <div style={{maxWidth:720,width:"100%",animation:"fadeUp .5s both",position:"relative",zIndex:1}}>
      <div style={{textAlign:"center",marginBottom:32}}>
        <div style={{fontSize:40,fontWeight:800,color:"#fff",letterSpacing:3,marginBottom:6,fontFamily:DS.font.heading}}>E<span style={{color:"#4F6AFF"}}>R</span>P</div>
        <div style={{fontSize:11,color:"rgba(255,255,255,0.35)",letterSpacing:4,textTransform:"uppercase"}}>ERP Platform</div>
        <div style={{width:60,height:2,background:"linear-gradient(90deg,transparent,#4F6AFF,transparent)",margin:"16px auto 24px"}}/>
        <div style={{fontSize:22,fontWeight:700,color:"#fff",fontFamily:DS.font.heading,marginBottom:6}}>Multi-Role Platform</div>
        <div style={{fontSize:13.5,color:"rgba(255,255,255,0.45)",fontFamily:DS.font.body}}>Pick your role to continue.</div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:mob?"1fr":"repeat(3,1fr)",gap:12}}>
        {ROLE_TOKENS.map(r=>(
          <button key={r.token} type="button" onClick={()=>pick(r.token)}
            style={{background:r.bg,border:`1px solid ${r.border}`,borderRadius:DS.radius.xl,padding:"22px 18px",cursor:"pointer",fontFamily:"inherit",textAlign:"left",transition:`all .25s ${DS.ease.snap}`,backdropFilter:"blur(12px)"}}
            onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-3px)";e.currentTarget.style.borderColor=r.color;e.currentTarget.style.boxShadow=`0 8px 24px ${r.color}33`}}
            onMouseLeave={e=>{e.currentTarget.style.transform="none";e.currentTarget.style.borderColor=r.border;e.currentTarget.style.boxShadow="none"}}>
            <div style={{fontSize:28,marginBottom:10}}>{r.icon}</div>
            <div style={{fontSize:15,fontWeight:700,color:r.color,fontFamily:DS.font.heading,marginBottom:4}}>{r.label}</div>
            <div style={{fontSize:11,color:"rgba(255,255,255,0.5)",lineHeight:1.5}}>{r.desc}</div>
          </button>
        ))}
      </div>
      <div style={{textAlign:"center",marginTop:28,fontSize:11,color:"rgba(255,255,255,0.25)",lineHeight:1.7}}>
        <div>© 2024 ERP Platform · v{APP_VERSION}</div>
        <div style={{marginTop:4,fontSize:10,color:"rgba(255,255,255,0.18)"}}>
          Designed &amp; built by <a href="https://github.com/Crewrafa" target="_blank" rel="noopener noreferrer" style={{color:"rgba(125,211,252,0.6)",textDecoration:"none"}}>Rafael Baez</a> · Portfolio showcase
        </div>
      </div>
    </div>
...
function Admin({user}){
  ...
  const[newUser,setNewUser]=useState({email:"",role:"recruiter",name:"",password:""});
  ...
  const addUser=async()=>{
    ...
    if(newUser.password.length<6)return show("⚠️ Password must be at least 6 characters");
    try{
      // 1. Create auth user with password (admin-only endpoint — send caller token)
      const authR=await fetch("/api/auth",{method:"POST",headers:ADM_AUTH(),body:JSON.stringify({action:"create_user",email:newUser.email,password:newUser.password,role:newUser.role,name:newUser.name})});
      const authD=await authR.json();if(!authR.ok)return show("⚠️ "+authD.error);
      // 2. Add to roles table
      const r=await fetch("/api/roles",{method:"POST",headers:ADM_AUTH(),body:JSON.stringify({action:"add",email:newUser.email,role:newUser.role,name:newUser.name})});
      const d=await r.json();if(d.error)return show("⚠️ "+d.error);
      setUsers(p=>[d,...p]);setNewUser({email:"",role:"recruiter",name:"",password:""});setShowAddUser(false);show("✅ User created with password");
  ...
  return(<div style={{minHeight:"100vh",background:`linear-gradient(180deg,${DS.surface.sunken} 0%,#EEF2F7 100%)`,fontFamily:DS.font.body,display:"flex"}}>
    ...
    <div style={{width:sideOpen?230:68,minHeight:"100vh",background:isRecruiter?"linear-gradient(180deg,#431407 0%,#7C2D12 50%,#9A3412 100%)":`linear-gradient(180deg,${DS.brand.navy900} 0%,#132B5E 50%,#1B3A70 100%)`,transition:`width .3s cubic-bezier(0.4,0,0.2,1)`,display:"flex",flexDirection:"column",flexShrink:0,position:isMob?"fixed":"sticky",top:0,left:0,zIndex:isMob?50:20,boxShadow:`${isMob?"0 0 40px rgba(0,0,0,0.4)":"1px 0 0 rgba(255,255,255,0.04)"}`,overflow:"hidden"}}>
      ...
      <div style={{padding:sideOpen?"22px 20px":"22px 0",display:"flex",alignItems:"center",justifyContent:sideOpen?"flex-start":"center",gap:10,borderBottom:"1px solid rgba(255,255,255,0.05)",minHeight:68}}>
        ...
        {sideOpen&&<div><div style={{fontSize:16,fontWeight:800,color:"#fff",letterSpacing:1,fontFamily:DS.font.heading}}>ERP<span style={{color:RC.accent}}>.</span></div><div style={{fontSize:8,color:"rgba(255,255,255,0.3)",letterSpacing:2.5,textTransform:"uppercase",fontFamily:DS.font.body}}>{RC.label}</div></div>}
      </div>
      {/* Nav items */}
        ...
        {showAddUser&&(<div style={{position:"fixed",inset:0,zIndex:50,background:"rgba(0,0,0,0.5)",display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
          <div style={{background:"#fff",borderRadius:DS.radius.xxl,padding:28,maxWidth:440,width:"100%",animation:"fadeUp .25s both",boxShadow:DS.shadow.lg}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:12}}>
              <div><div style={{fontSize:11,fontWeight:600,color:DS.text.muted,marginBottom:4}}>Email *</div><input type="email" value={newUser.email} onChange={e=>setNewUser({...newUser,email:e.target.value})} placeholder="e.g. ana.rios@erp-platform.local" style={{width:"100%",border:`1.5px solid ${DS.surface.border}`,borderRadius:DS.radius.sm,padding:"10px 14px",fontSize:13,fontFamily:DS.font.body}}/></div>
              <div><div style={{fontSize:11,fontWeight:600,color:DS.text.muted,marginBottom:4}}>Password *</div><input type="password" value={newUser.password} onChange={e=>setNewUser({...newUser,password:e.target.value})} placeholder="e.g. choose a strong one — min 6 chars" style={{width:"100%",border:`1.5px solid ${DS.surface.border}`,borderRadius:DS.radius.sm,padding:"10px 14px",fontSize:13,fontFamily:DS.font.body}}/></div>
              <div><div style={{fontSize:11,fontWeight:600,color:DS.text.muted,marginBottom:4}}>Name</div><input type="text" value={newUser.name} onChange={e=>setNewUser({...newUser,name:e.target.value})} placeholder="e.g. Ana Ríos" style={{width:"100%",border:`1.5px solid ${DS.surface.border}`,borderRadius:DS.radius.sm,padding:"10px 14px",fontSize:13,fontFamily:DS.font.body}}/></div>
              ...
              <button type="button" onClick={addUser} disabled={!newUser.email.includes("@")||newUser.password.length<6} style={{width:"100%",marginTop:8,background:newUser.email.includes("@")&&newUser.password.length>=6?`linear-gradient(135deg,${DS.brand.navy900},${DS.brand.blue700})`:"#e2e8f0",color:newUser.email.includes("@")&&newUser.password.length>=6?"#fff":DS.text.faint,border:"none",borderRadius:DS.radius.md,padding:14,fontSize:14,fontWeight:600,fontFamily:DS.font.heading,cursor:newUser.email.includes("@")&&newUser.password.length>=6?"pointer":"default",boxShadow:newUser.email.includes("@")&&newUser.password.length>=6?DS.shadow.blue:"none"}}>Add User</button>
            </div>
          </div>
        </div>)}
      </div>)}
        ...
        {!selP?(<>
          ...
          <div style={{display:"grid",gridTemplateColumns:isMob?"1fr":"1fr 1fr",gap:20}}>
            ...
            <div>
              ...
              <textarea value={revNotes} onChange={e=>setRevNotes(e.target.value)} rows={3} placeholder="e.g. 'Stack matches well — Java/Spring Boot + AWS + PostgreSQL confirmed. Worth a soft-skills round.' (Required when rejecting.)" style={{width:"100%",border:`1.5px solid ${DS.surface.border}`,borderRadius:DS.radius.lg,padding:"12px 14px",fontSize:13,fontFamily:DS.font.body,outline:"none",resize:"vertical",color:DS.text.body,lineHeight:1.5,background:"#fff",marginBottom:12}}/>
              ...
              {revQuestions.map((q,i)=>(<div key={i} style={{display:"flex",gap:6,marginBottom:6}}><span style={{fontSize:11,color:DS.brand.blue700,paddingTop:8,fontWeight:700}}>{i+1}.</span><textarea value={q} onChange={e=>{const n=[...revQuestions];n[i]=e.target.value;setRevQuestions(n)}} rows={2} placeholder="e.g. 'How do they handle disagreement with a more senior engineer on technical direction?'" style={{flex:1,border:`1px solid ${DS.surface.border}`,borderRadius:DS.radius.sm,padding:"8px 10px",fontSize:12,fontFamily:DS.font.body,outline:"none",resize:"vertical"}}/><button type="button" onClick={()=>setRevQuestions(p=>p.filter((_,j)=>j!==i))} style={{background:"none",border:"none",color:"#dc2626",cursor:"pointer",fontSize:14,paddingTop:6}}>✕</button></div>))}
      ...
      {view==="talent_pool"&&(<div style={{animation:"fadeUp .3s both"}}>
          ...
          <div style={{background:"#fff",borderRadius:DS.radius.xxl,maxWidth:520,width:"100%",maxHeight:"85vh",overflowY:"auto",animation:"fadeUp .3s both",boxShadow:DS.shadow.lg}}>
            ...
            <div style={{padding:"24px 28px"}}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                <input type="text" value={newCand.name} onChange={e=>setNewCand({...newCand,name:e.target.value})} placeholder="e.g. Carlos Méndez *" style={IS}/>
                <input type="email" value={newCand.email} onChange={e=>setNewCand({...newCand,email:e.target.value})} placeholder="e.g. carlos.mendez@gmail.com" style={IS}/>
                ...
                <input type="text" value={newCand.location} onChange={e=>setNewCand({...newCand,location:e.target.value})} placeholder="e.g. Bogotá, Colombia · Remote" style={IS}/>
                ...
                <input type="text" value={newCand.experience} onChange={e=>setNewCand({...newCand,experience:e.target.value})} placeholder="e.g. 6 years (Senior)" style={IS}/>
              </div>
              <div style={{marginTop:10,display:"flex",gap:6}}><input type="text" value={skillInput} onChange={e=>setSkillInput(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&skillInput.trim()){setNewCand({...newCand,skills:[...newCand.skills,skillInput.trim()]});setSkillInput("")}}} placeholder="e.g. PostgreSQL — press Enter to add" style={{...IS,flex:1}}/></div>
              ...
              <textarea value={newCand.notes} onChange={e=>setNewCand({...newCand,notes:e.target.value})} placeholder="e.g. '5y at fintech, comfortable with on-call. Strong English (C1). Available in 2 weeks.'" rows={2} style={{...IS,marginTop:10,resize:"none"}}/>
        ...
        {editCand&&(<div style={{position:"fixed",inset:0,zIndex:50,background:"rgba(15,23,42,0.6)",backdropFilter:"blur(4px)",display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
          <div style={{background:"#fff",borderRadius:DS.radius.xxl,maxWidth:520,width:"100%",maxHeight:"85vh",overflowY:"auto",animation:"fadeUp .3s both",boxShadow:DS.shadow.lg}}>
            ...
            <div style={{padding:"24px 28px"}}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                <input type="text" value={editCand.name||""} onChange={e=>setEditCand({...editCand,name:e.target.value})} placeholder="e.g. Carlos Méndez *" style={IS}/>
                <input type="email" value={editCand.email||""} onChange={e=>setEditCand({...editCand,email:e.target.value})} placeholder="e.g. carlos.mendez@gmail.com" style={IS}/>
                ...
                <input type="text" value={editCand.location||""} onChange={e=>setEditCand({...editCand,location:e.target.value})} placeholder="e.g. Bogotá, Colombia · Remote" style={IS}/>
                ...
                <input type="text" value={editCand.experience||""} onChange={e=>setEditCand({...editCand,experience:e.target.value})} placeholder="e.g. 6 years (Senior)" style={IS}/>
              </div>
              <div style={{marginTop:10,display:"flex",gap:6}}><input type="text" value={editSkillInput} onChange={e=>setEditSkillInput(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&editSkillInput.trim()){setEditCand({...editCand,skills:[...(editCand.skills||[]),editSkillInput.trim()]});setEditSkillInput("")}}} placeholder="e.g. PostgreSQL — press Enter to add" style={{...IS,flex:1}}/></div>
              ...
              <textarea value={editCand.notes||""} onChange={e=>setEditCand({...editCand,notes:e.target.value})} placeholder="e.g. 'Updated availability — can start in 2 weeks instead of 4.'" rows={2} style={{...IS,marginTop:10,resize:"none"}}/>
    ...
    <div style={{background:"linear-gradient(135deg,#2E1065,#5B21B6,#7C3AED)",padding:"0",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{maxWidth:900,margin:"0 auto",padding:"18px 24px",display:"flex",justifyContent:"space-between",alignItems:"center",position:"relative",zIndex:1}}>
        <div style={{display:"flex",alignItems:"center",gap:14}}>
          <span style={{fontSize:22,fontWeight:800,color:"#fff",letterSpacing:1.5,fontFamily:DS.font.heading}}>ERP<span style={{color:"#c084fc"}}>.</span></span>
      ...
      {view==="detail"&&selP&&(<div style={{animation:"fadeUp .3s both"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:isMob?"1fr":"1fr 1fr",gap:20}}>
          ...
          <div>
            ...
            <textarea value={notes} onChange={e=>setNotes(e.target.value)} rows={6} placeholder="e.g. 'Client says the candidate was articulate and confident in the interview. Comfortable pushing back on requirements when needed. Mentioned mentoring 3 juniors at last job — clear leadership instinct. Some hesitation around stakeholder management. Strong cultural fit — values pragmatism over perfection.'" style={{width:"100%",border:`1.5px solid ${DS.surface.border}`,borderRadius:DS.radius.lg,padding:"14px 16px",fontSize:13,fontFamily:DS.font.body,outline:"none",resize:"vertical",color:DS.text.body,lineHeight:1.6,background:"#fff"}}/>
            ...
            <div style={{marginTop:16}}>
              ...
              <AIChat systemPrompt={AI_PROMPTS.ana_chat} placeholder="Try: 'What traits suggest a strong tech lead?' or 'Summarize the soft-skills risks'"/>
            </div>
          </div>
        </div>
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    // Short-circuit dev tokens client-side (works without /api/* — needed for plain `vite` dev)
    const DEV_TOKEN_MAP={
      "dev":{email:"psicologorafaelbaez@gmail.com",id:"00000000-0000-0000-0000-000000000001",role:"admin"},
      "dev-sales":{email:"sales@erp-platform.local",id:"00000000-0000-0000-0000-000000000005",role:"sales"},
      "dev-recruiter":{email:"recruiter@erp-platform.local",id:"00000000-0000-0000-0000-000000000004",role:"recruiter"},
      "dev-ana":{email:"ana@erp-platform.local",id:"00000000-0000-0000-0000-000000000003",role:"ana"},
      "dev-finance":{email:"finance@erp-platform.local",id:"00000000-0000-0000-0000-000000000006",role:"finance"},
      "dev-client":{email:"client@test.com",id:"00000000-0000-0000-0000-000000000002",role:"client"},
    };
    if(DEV_TOKEN_MAP[token]){
      const u=DEV_TOKEN_MAP[token];
      setAuth({user:{email:u.email,id:u.id},token});setUserRole(u.role);setALd(false);return;
    }
    // Resolve role via API for real Supabase tokens
    (async()=>{
      try{
...
function ClientForm({user}){
  ...
  const[cName,setCName]=useState("Sarah Chen");const[cComp,setCComp]=useState("Northwind Labs");const cEmail=user.email;
  ...
  const[cat,setCat]=useState("technical");const[role,setRole]=useState("Backend Developer");const[cRole,setCRole]=useState("");const[rSrch,setRSrch]=useState("");
  const[sen,setSen]=useState("Senior");const[exp,setExp]=useState("4-6 years");const[hc,setHc]=useState(2);
  const[eng,setEng]=useState("Full-time (40h/week)");const[sTime,setSTime]=useState("9:00");const[eTime,setETime]=useState("17:00");const[wHrs,setWHrs]=useState(40);const[tz,setTz]=useState("COT (UTC-5)");
  const[langs,setLangs]=useState(["Java","SQL"]);const[vers,setVers]=useState({Java:["17","21"]});const[fws,setFws]=useState(["Spring Boot","Spring Security"]);const[fwVers,setFwVers]=useState({"Spring Boot":["3.2","3.3"]});
  const[clds,setClds]=useState(["AWS"]);const[cldSvcs,setCldSvcs]=useState(["EC2","S3","Lambda","RDS","EKS"]);const[otherCloud,setOtherCloud]=useState("");
  const[dbs,setDbs]=useState(["PostgreSQL","Redis"]);const[dbVers,setDbVers]=useState({PostgreSQL:["15","16"]});const[dvps,setDvps]=useState(["Docker","Kubernetes","GitHub Actions","Terraform"]);const[dvpVers,setDvpVers]=useState({Kubernetes:["1.29","1.30"]});
  const[erps,setErps]=useState([]);const[erpVers,setErpVers]=useState({});const[qas,setQas]=useState([]);const[oTech,setOTech]=useState("Kafka, Datadog");
  const[mh,setMh]=useState(["Java","Spring Boot","PostgreSQL","AWS","Docker"]);const[nh,setNh]=useState(["Kubernetes","Terraform","Kafka","Redis"]);const[showAll,setShowAll]=useState(false);
  const[engl,setEngl]=useState("B2 - Upper Intermediate");const[loc,setLoc]=useState("LATAM");const[acad,setAcad]=useState("Bachelor's");const[certs,setCerts]=useState("AWS Solutions Architect Associate");
  const[meths,setMeths]=useState(["Scrum"]);const[inds,setInds]=useState(["Fintech"]);
  const[aiTs,setAiTs]=useState(["Claude","GitHub Copilot","Cursor"]);const[otherAI,setOtherAI]=useState("");
  const[profTools,setProfTools]=useState([]); // professional tools for non-tech categories
  const[visa,setVisa]=useState("Not required");const[travel,setTravel]=useState("None");const[notes,setNotes]=useState("Fully remote team across PST/COT timezones. Replacing a Senior engineer who left for FAANG. Start ASAP — ideally within 30 days.");
  ...
  // ═══════════ STEPS ═══════════
  const s0=()=>(<StaggerIn base={0.08}><div style={{textAlign:"center",marginBottom:32}}><div style={{width:72,height:72,borderRadius:"50%",background:`linear-gradient(135deg,${DS.brand.blue50},#e0f2fe)`,display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 16px",fontSize:36,boxShadow:"0 4px 16px rgba(27,111,232,0.1)"}}>👋</div><div style={{fontSize:24,fontWeight:700,color:DS.text.h1,fontFamily:DS.font.heading,letterSpacing:"-0.025em"}}>Welcome to the ERP Platform</div><div style={{fontSize:14,color:DS.text.muted,marginTop:8,fontFamily:DS.font.body}}>Signed in as <span style={{color:DS.brand.blue700,fontWeight:500}}>{cEmail}</span></div></div><InfoBox text={STEP_INFO.Client}/><div style={{maxWidth:480,margin:"0 auto"}}><Section title="Your Name *" delay={0}><input type="text" value={cName} onChange={e=>setCName(e.target.value)} placeholder="e.g. Sarah Chen" style={IS} onFocus={e=>{e.target.style.borderColor=DS.brand.blue700;e.target.style.boxShadow=DS.shadow.glow}} onBlur={e=>{e.target.style.borderColor=DS.surface.border;e.target.style.boxShadow="none"}}/></Section><Section title="Company *" delay={0}><input type="text" value={cComp} onChange={e=>setCComp(e.target.value)} placeholder="e.g. Northwind Labs" style={IS} onFocus={e=>{e.target.style.borderColor=DS.brand.blue700;e.target.style.boxShadow=DS.shadow.glow}} onBlur={e=>{e.target.style.borderColor=DS.surface.border;e.target.style.boxShadow="none"}}/></Section></div></StaggerIn>);
  ...
  const s4=()=>{
    ...
    if(isProfCat){
      ...
      return(<><InfoBox text={STEP_INFO.Stack}/>
        ...
        <Section title="Other Tools or Skills" delay={0.35}><input type="text" value={oTech} onChange={e=>setOTech(e.target.value)} placeholder="e.g. Notion, Linear, Datadog, Stripe..." style={IS}/></Section>
      </>);
    }
    ...
    <Section title="Other Technologies" delay={0.3}><input type="text" value={oTech} onChange={e=>setOTech(e.target.value)} placeholder="e.g. Kafka, RabbitMQ, GraphQL, gRPC, Temporal..." style={IS}/></Section>
    ...
    <Section title="AI Tools" sub="Select tools used by the team" delay={0.34}><div style={{display:"flex",flexWrap:"wrap",gap:8}}>{AI_TOOLS.map(t=><Pill key={t} label={t} selected={aiTs.includes(t)} onClick={()=>setAiTs(p=>toggle(p,t))} color="#06b6d4" small/>)}</div><input type="text" value={otherAI} onChange={e=>setOtherAI(e.target.value)} placeholder="e.g. v0 by Vercel, Codeium, JetBrains AI..." style={{...IS,marginTop:10,maxWidth:300}}/></Section>
    <Section title="Certifications" sub="Optional" delay={0.36}><input type="text" value={certs} onChange={e=>setCerts(e.target.value)} placeholder="e.g. AWS Solutions Architect, PMP, CISSP, Kubernetes CKA..." style={IS}/></Section>
  </>)};
  ...
  const s6=()=>(<><InfoBox text={STEP_INFO.Details}/>
    ...
    <Section title="Additional Notes" delay={0.33}><textarea value={notes} onChange={e=>setNotes(e.target.value)} rows={3} placeholder="e.g. 'Fully remote team in PST timezone. Replacing a Senior who left for FAANG. Start ASAP.'" style={{...IS,resize:"vertical"}}/></Section>
  </>);
      ...
      <div style={{background:"#fff",borderRadius:DS.radius.xl,padding:"24px 28px",marginBottom:16,border:`1px solid ${DS.surface.border}`,boxShadow:DS.shadow.sm}}>
        ...
        :<>
          <div style={{marginBottom:16}}>
            ...
            <textarea value={aiObjective} onChange={e=>setAiObjective(e.target.value)} rows={3} placeholder="e.g. 'Design and own the backend services powering our checkout flow — scaling from 100 to 10k req/sec on Java + Spring Boot + PostgreSQL on AWS. Mentor 2 mid-level engineers.'" style={{width:"100%",border:`1.5px solid ${DS.surface.border}`,borderRadius:DS.radius.md,padding:"12px 14px",fontSize:13,fontFamily:DS.font.body,outline:"none",resize:"vertical",color:DS.text.body,lineHeight:1.6,background:DS.surface.page}}/>
          </div>
          <div style={{marginBottom:12}}>
            ...
            {aiResponsibilities.map((r,i)=>(
              <div key={i} style={{display:"flex",gap:8,marginBottom:10,alignItems:"flex-start"}}>
                ...
                <textarea value={r} onChange={e=>{const n=[...aiResponsibilities];n[i]=e.target.value;setAiResponsibilities(n)}} rows={3} placeholder="e.g. 'System Architecture: Design and maintain scalable microservices using Spring Boot and AWS EKS.'" style={{flex:1,border:`1.5px solid ${DS.surface.border}`,borderRadius:DS.radius.md,padding:"12px 14px",fontSize:13,fontFamily:DS.font.body,outline:"none",color:DS.text.body,background:DS.surface.page,lineHeight:1.6,resize:"vertical"}}/>
    ...
    {open&&<div style={{position:"fixed",bottom:24,right:24,width:380,maxHeight:520,borderRadius:DS.radius.xxl,overflow:"hidden",boxShadow:DS.shadow.lg,zIndex:50,animation:"fadeUp .25s both",display:"flex",flexDirection:"column",background:"#fff",border:`1px solid ${DS.surface.border}`}}>
      <div style={{background:`linear-gradient(135deg,${DS.brand.navy900},${DS.brand.blue700})`,padding:"14px 18px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div><div style={{fontSize:13,fontWeight:600,color:"#fff",fontFamily:DS.font.heading}}>ERP Assistant</div><div style={{fontSize:10,color:"rgba(255,255,255,0.5)"}}>Smart staffing help</div></div>
        </div>
      ...
      <div style={{flex:1,overflow:"hidden"}}><AIChat systemPrompt={AI_PROMPTS.platform_help+ctxStr} placeholder="Try: 'What's missing from my stack?' or 'Suggest 3 must-have skills'"/></div>
    </div>}
  </>);
}
...
function SalesModule({user}){
  ...
  const sendInvite=(email)=>{
    ...
    const subject=encodeURIComponent("ERP Platform — Define Your Ideal Tech Talent");
    const body=encodeURIComponent(`Hi,\n\nI'd like to invite you to submit your staffing requirements through our ERP Platform.\n\nClick here to get started:\n${clientLink}\n\nThe process takes about 5 minutes and our AI will help you define the perfect tech profile.\n\nBest regards,\n${user.email?.split("@")[0]||"ERP Team"}\nERP Platform`);
  ...
  const sendReviewLink=(profileId,email)=>{
    ...
    const subject=encodeURIComponent("ERP Platform — Your candidates are ready for review");
  ...
  return(<div style={{minHeight:"100vh",background:DS.surface.page,fontFamily:DS.font.body,display:"flex",flexDirection:"column"}}>
    ...
    <div style={{background:DS.brand.navy900}}>
      <div style={{maxWidth:1100,margin:"0 auto",padding:"14px 24px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div style={{display:"flex",alignItems:"center",gap:14}}>
          <span style={{fontSize:22,fontWeight:800,color:"#fff",letterSpacing:1.5,fontFamily:DS.font.heading}}>ERP<span style={{color:"#6EE7B7"}}>.</span></span>
    ...
    {tab==="clients"&&(<div style={{animation:"fadeUp .3s both"}}>
      ...
      <div style={{background:"#fff",borderRadius:DS.radius.xl,padding:"22px 24px",marginBottom:20,border:`1px solid ${DS.surface.border}`,boxShadow:DS.shadow.sm}}>
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
          <input type="email" value={inviteEmail} onChange={e=>setInviteEmail(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")sendInvite()}} placeholder="e.g. sarah.chen@northwind.io" style={{flex:1,minWidth:200,border:`1.5px solid ${DS.surface.border}`,borderRadius:DS.radius.md,padding:"11px 14px",fontSize:13,fontFamily:DS.font.body,outline:"none"}} onFocus={e=>{e.target.style.borderColor="#059669"}} onBlur={e=>{e.target.style.borderColor=DS.surface.border}}/>
    ...
    {tab==="dashboard"&&(<div style={{animation:"fadeUp .3s both"}}>
      {/* Sales-specific KPIs */}
      {(()=>{
        const authorizedCount=ps.filter(p=>p.profile_data?.reviewAuthorized).length;
        const acceptedCands=allAssigns.filter(a=>a.client_decision==="accepted").length;
        const rejectedCands=allAssigns.filter(a=>a.client_decision==="rejected").length;
        const pendingCands=allAssigns.filter(a=>!a.client_decision||a.client_decision==="pending").length;
        const readyForClient=ps.filter(p=>p.status==="filled").length;
        return(<>
      <div style={{display:"grid",gridTemplateColumns:isMob?"repeat(2,1fr)":"repeat(5,1fr)",gap:10,marginBottom:24}}>
        {[{l:"Clients",v:uniqueClients,c:"#8b5cf6"},{l:"Profiles",v:totalProfiles,c:"#1B6FE8"},{l:"Links Sent",v:authorizedCount,c:"#059669"},{l:"Ready for Client",v:readyForClient,c:"#f59e0b"},{l:"Client Responses",v:acceptedCands+rejectedCands,c:"#0369a1"}].map((kpi,i)=>(
      ...
      <div style={{display:"grid",gridTemplateColumns:isMob?"1fr":"1fr 1fr",gap:16}}>
        ...
        <div style={{background:"#fff",borderRadius:DS.radius.xl,padding:"20px 22px",border:`1px solid ${DS.surface.border}`,boxShadow:DS.shadow.sm}}>
          <div style={{fontSize:14,fontWeight:700,color:DS.text.h1,fontFamily:DS.font.heading,marginBottom:14}}>Client Decisions</div>
          {[{l:"Accepted by Client",v:acceptedCands,c:"#059669"},{l:"Rejected by Client",v:rejectedCands,c:"#dc2626"},{l:"Pending Decision",v:pendingCands,c:"#f59e0b"},{l:"Review Links Live",v:authorizedCount,c:"#1B6FE8"}].map((m,i)=>{const pct=(acceptedCands+rejectedCands+pendingCands)?Math.round((m.v/(acceptedCands+rejectedCands+pendingCands))*100):0;return(
            <div key={i} style={{marginBottom:10}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}><span style={{fontSize:11,color:DS.text.body}}>{m.l}</span><span style={{fontSize:12,fontWeight:700,color:m.c}}>{m.v}</span></div>
            <div style={{height:4,background:DS.surface.sunken,borderRadius:2,overflow:"hidden"}}><div style={{height:"100%",width:`${pct}%`,background:m.c,borderRadius:2}}/></div></div>
          )})}
        </div>
        {/* By client */}
        <div style={{background:"#fff",borderRadius:DS.radius.xl,padding:"20px 22px",border:`1px solid ${DS.surface.border}`,boxShadow:DS.shadow.sm}}>
          <div style={{fontSize:14,fontWeight:700,color:DS.text.h1,fontFamily:DS.font.heading,marginBottom:14}}>Profiles by Client</div>
          {clients.map(company=>{const cps=ps.filter(p=>p.client_company===company);const ready=cps.filter(p=>p.profile_data?.reviewAuthorized).length;return(
            <div key={company} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",borderBottom:`1px solid ${DS.surface.borderLight}`}}>
              <span style={{fontSize:13,fontWeight:500,color:DS.text.body}}>{company}</span>
              <div style={{display:"flex",gap:8,alignItems:"center"}}><span style={{fontSize:10,color:DS.text.faint}}>{ready} live</span><span style={{fontSize:14,fontWeight:700,color:DS.text.h1}}>{cps.length}</span></div>
            </div>
          )})}
        </div>
      </div>
...
function FinanceModule({user}){
  const isMob=useW()<640;
  const[profiles,setProfiles]=useState([]);const[pricing,setPricing]=useState([]);const[ld,setLd]=useState(true);
  const[toast,setToast]=useState("");const[selProfile,setSelProfile]=useState(null);
  const[form,setForm]=useState({client_rate:5000,rockstar_salary:2500,resources:1,contract_months:12,vacation_days:10,holiday_country:"Mexico",notes:""});
  const[saving,setSaving]=useState(false);
  const show=(m)=>{setToast(m);setTimeout(()=>setToast(""),3500)};

  useEffect(()=>{(async()=>{try{
    const r=await fetch("/api/admin",{method:"POST",headers:{"Authorization":`Bearer ${localStorage.getItem("sb-access-token")}`,"Content-Type":"application/json"},body:JSON.stringify({action:"list_all_pricing"})});
    if(r.ok){const d=await r.json();setProfiles(d.profiles||[]);setPricing(d.pricing||[])}
  }catch(e){console.warn(e)}finally{setLd(false)}})()},[]);

  const getPricing=(pid)=>pricing.find(p=>p.profile_id===pid);
  const savePricing=async()=>{if(!selProfile)return;setSaving(true);try{
    await fetch("/api/admin",{method:"POST",headers:{"Authorization":`Bearer ${localStorage.getItem("sb-access-token")}`,"Content-Type":"application/json"},body:JSON.stringify({action:"save_pricing",profile_id:selProfile.id,...form})});
    setPricing(p=>{const idx=p.findIndex(x=>x.profile_id===selProfile.id);const item={...form,profile_id:selProfile.id};if(idx>=0){const n=[...p];n[idx]=item;return n}return[...p,item]});
    show("✅ Pricing saved");
  }catch(e){show("⚠️ "+e.message)}finally{setSaving(false)}};

  const HOLIDAYS={Mexico:{0:1,1:1,2:1,3:1,4:1,5:0,6:0,7:0,8:1,9:0,10:1,11:2},USA:{0:2,1:1,2:0,3:0,4:1,5:0,6:1,7:0,8:1,9:1,10:2,11:1},Canada:{0:1,1:1,2:0,3:1,4:1,5:0,6:1,7:1,8:1,9:1,10:1,11:1}};
  const calcMargin=(pr)=>{if(!pr)return{margin:0,pct:0,revenue:0,cost:0};const rev=(pr.client_rate||0)*(pr.resources||1)*(pr.contract_months||12);const cost=(pr.rockstar_salary||0)*(pr.resources||1)*(pr.contract_months||12);return{margin:rev-cost,pct:rev?Math.round(((rev-cost)/rev)*100):0,revenue:rev,cost}};
  const fmt=(n)=>"$"+Math.round(n).toLocaleString();
  const IS={width:"100%",border:`1.5px solid ${DS.surface.border}`,borderRadius:DS.radius.sm,padding:"10px 14px",fontSize:13,fontFamily:DS.font.body,outline:"none",background:"#fff"};
  const clients=[...new Set(profiles.map(p=>p.client_company).filter(Boolean))];

  // Overall totals
  const allCalcs=profiles.map(p=>{const pr=getPricing(p.id);return{...p,calc:calcMargin(pr),pr}});
  const totalRev=allCalcs.reduce((s,p)=>s+p.calc.revenue,0);
  const totalCost=allCalcs.reduce((s,p)=>s+p.calc.cost,0);
  const totalMargin=totalRev-totalCost;const totalPct=totalRev?Math.round((totalMargin/totalRev)*100):0;
  const pricedCount=allCalcs.filter(p=>p.pr).length;
  ...
  return(<div style={{minHeight:"100vh",background:`linear-gradient(160deg,#F0F9FF,#E0F2FE 30%,${DS.surface.page})`,fontFamily:DS.font.body,display:"flex",flexDirection:"column"}}>
    {toast&&<div style={{position:"fixed",top:20,left:"50%",transform:"translateX(-50%)",zIndex:100,background:toast.includes("⚠️")?"#dc2626":"#0D2550",color:"#fff",padding:"12px 28px",borderRadius:DS.radius.lg,fontSize:13,fontWeight:500,boxShadow:DS.shadow.lg,animation:"fadeUp .3s both"}}>{toast}</div>}
    <div style={{background:"linear-gradient(135deg,#0C4A6E,#0369A1,#0EA5E9)",position:"relative",overflow:"hidden"}}>
      <HeaderBG/>
      <div style={{maxWidth:1100,margin:"0 auto",padding:"18px 24px",display:"flex",justifyContent:"space-between",alignItems:"center",position:"relative",zIndex:1}}>
        <div style={{display:"flex",alignItems:"center",gap:14}}>
          <span style={{fontSize:22,fontWeight:800,color:"#fff",letterSpacing:1.5,fontFamily:DS.font.heading}}>ERP<span style={{color:"#7DD3FC"}}>.</span></span>
          <div style={{height:20,width:1,background:"rgba(255,255,255,0.15)"}}/>
          <span style={{fontSize:13,fontWeight:600,color:"rgba(125,211,252,0.8)",fontFamily:DS.font.heading}}>Finance — Revenue by Profile</span>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <span style={{fontSize:12,color:"rgba(255,255,255,0.7)",fontFamily:DS.font.body}}>{user.email}</span>
          <button onClick={signOut} type="button" style={{fontSize:11,background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.15)",borderRadius:DS.radius.md,padding:"7px 14px",color:"rgba(255,255,255,0.7)",cursor:"pointer",fontFamily:DS.font.body}}>Sign Out</button>
    ...
    <div style={{maxWidth:1100,margin:"0 auto",padding:"24px",flex:1,width:"100%"}}>
      {/* KPIs */}
      <div style={{display:"grid",gridTemplateColumns:isMob?"repeat(2,1fr)":"repeat(5,1fr)",gap:10,marginBottom:24}}>
        {[{l:"Total Revenue",v:fmt(totalRev),c:"#0369A1"},{l:"Total Cost",v:fmt(totalCost),c:"#dc2626"},{l:"Net Margin",v:fmt(totalMargin),c:totalMargin>0?"#059669":"#dc2626"},{l:"Avg Margin",v:`${totalPct}%`,c:totalPct>=40?"#059669":totalPct>=20?"#f59e0b":"#dc2626"},{l:"Priced",v:`${pricedCount}/${profiles.length}`,c:"#8b5cf6"}].map((kpi,i)=>(
          <div key={i} style={{background:"#fff",borderRadius:DS.radius.lg,padding:"18px 16px",border:`1px solid ${DS.surface.border}`,boxShadow:DS.shadow.sm}}>
            <div style={{fontSize:10,color:DS.text.faint,fontWeight:600,textTransform:"uppercase",letterSpacing:"0.05em"}}>{kpi.l}</div>
            <div style={{fontSize:22,fontWeight:800,color:kpi.c,fontFamily:DS.font.heading,lineHeight:1,marginTop:6}}>{kpi.v}</div>
          </div>
        ))}
      </div>
      <div style={{display:"grid",gridTemplateColumns:isMob?"1fr":selProfile?"1fr 380px":"1fr",gap:20}}>
        {/* Profile list by client */}
        <div>
          {clients.map(company=>{const cps=allCalcs.filter(p=>p.client_company===company);const cRev=cps.reduce((s,p)=>s+p.calc.revenue,0);const cMargin=cps.reduce((s,p)=>s+p.calc.margin,0);const cPct=cRev?Math.round((cMargin/cRev)*100):0;
          return(<div key={company} style={{marginBottom:16}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
              <div style={{fontSize:15,fontWeight:700,color:DS.text.h1,fontFamily:DS.font.heading}}>{company}</div>
              <div style={{display:"flex",gap:12,fontSize:11,color:DS.text.muted}}>
                <span>Rev: <strong style={{color:"#0369A1"}}>{fmt(cRev)}</strong></span>
                <span>Margin: <strong style={{color:cMargin>0?"#059669":"#dc2626"}}>{fmt(cMargin)} ({cPct}%)</strong></span>
              </div>
            </div>
            {cps.map((p,i)=>{const pr=p.pr;const m=p.calc;const viable=m.pct>=40?"💰":m.pct>=20?"⚠️":m.pct>0?"🔴":"—";
              return(<div key={p.id} onClick={()=>{setSelProfile(p);const existing=getPricing(p.id);setForm(existing?{client_rate:existing.client_rate||5000,rockstar_salary:existing.rockstar_salary||2500,resources:existing.resources||1,contract_months:existing.contract_months||12,vacation_days:existing.vacation_days??10,holiday_country:existing.holiday_country||"Mexico",notes:existing.notes||""}:{client_rate:5000,rockstar_salary:2500,resources:1,contract_months:12,vacation_days:10,holiday_country:"Mexico",notes:""})}} style={{background:selProfile?.id===p.id?"#f0f9ff":"#fff",borderRadius:DS.radius.lg,padding:"14px 18px",marginBottom:6,border:`1px solid ${selProfile?.id===p.id?"#0369A1":DS.surface.border}`,cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",transition:"all .2s",boxShadow:DS.shadow.sm}} onMouseEnter={e=>{if(selProfile?.id!==p.id)e.currentTarget.style.borderColor="#93c5fd"}} onMouseLeave={e=>{if(selProfile?.id!==p.id)e.currentTarget.style.borderColor=DS.surface.border}}>
                <div>
                  <div style={{fontSize:13,fontWeight:600,color:DS.text.h1,fontFamily:DS.font.heading}}>{p.role} — {p.seniority}</div>
                  <div style={{fontSize:11,color:DS.text.muted,marginTop:2}}>{p.client_name} · {STATUS_LABELS[p.status]||p.status}</div>
                </div>
                <div style={{display:"flex",alignItems:"center",gap:12,flexShrink:0}}>
                  {pr?<>
                    <div style={{textAlign:"right"}}><div style={{fontSize:12,fontWeight:700,color:m.margin>0?"#059669":"#dc2626"}}>{fmt(m.margin)}</div><div style={{fontSize:9,color:DS.text.faint}}>{m.pct}% margin</div></div>
                    <span style={{fontSize:16}}>{viable}</span>
                  </>:<span style={{fontSize:10,color:DS.text.placeholder,background:DS.surface.sunken,padding:"4px 10px",borderRadius:DS.radius.pill}}>Not priced</span>}
                </div>
              </div>);})}
          </div>);})}
          {!profiles.length&&<div style={{textAlign:"center",padding:48,background:"#fff",borderRadius:DS.radius.xl,border:`1px solid ${DS.surface.border}`,color:DS.text.faint}}><div style={{fontSize:36,marginBottom:10}}>💰</div>No profiles yet</div>}
        </div>
        {/* Pricing form */}
        {selProfile&&<div style={{background:"#fff",borderRadius:DS.radius.xl,padding:"22px",border:`1px solid ${DS.surface.border}`,boxShadow:DS.shadow.sm,alignSelf:"flex-start",position:"sticky",top:24}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16}}>
            <div><div style={{fontSize:15,fontWeight:700,color:DS.text.h1,fontFamily:DS.font.heading}}>{selProfile.role}</div><div style={{fontSize:11,color:DS.text.muted,marginTop:2}}>{selProfile.client_company} · {selProfile.seniority}</div></div>
            <button type="button" onClick={()=>setSelProfile(null)} style={{background:"none",border:"none",fontSize:16,cursor:"pointer",color:DS.text.faint}}>✕</button>
          </div>
          <div style={{display:"grid",gap:12}}>
            <div><div style={{fontSize:11,fontWeight:600,color:DS.text.muted,marginBottom:4}}>Client Rate (USD/month)</div><input type="number" value={form.client_rate} onChange={e=>setForm({...form,client_rate:+e.target.value||0})} style={IS}/><input type="range" min={1000} max={20000} step={100} value={form.client_rate} onChange={e=>setForm({...form,client_rate:+e.target.value})} style={{width:"100%",accentColor:"#0369A1",marginTop:4}}/></div>
            <div><div style={{fontSize:11,fontWeight:600,color:DS.text.muted,marginBottom:4}}>Rockstar Salary (USD/month)</div><input type="number" value={form.rockstar_salary} onChange={e=>setForm({...form,rockstar_salary:+e.target.value||0})} style={IS}/><input type="range" min={500} max={15000} step={100} value={form.rockstar_salary} onChange={e=>setForm({...form,rockstar_salary:+e.target.value})} style={{width:"100%",accentColor:"#dc2626",marginTop:4}}/></div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              <div><div style={{fontSize:11,fontWeight:600,color:DS.text.muted,marginBottom:4}}>Resources</div><input type="number" min={1} max={50} value={form.resources} onChange={e=>setForm({...form,resources:Math.max(1,+e.target.value||1)})} style={IS}/></div>
              <div><div style={{fontSize:11,fontWeight:600,color:DS.text.muted,marginBottom:4}}>Months</div><input type="number" min={1} max={36} value={form.contract_months} onChange={e=>setForm({...form,contract_months:Math.max(1,+e.target.value||1)})} style={IS}/></div>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              <div><div style={{fontSize:11,fontWeight:600,color:DS.text.muted,marginBottom:4}}>Vacation</div><input type="number" min={0} max={30} value={form.vacation_days} onChange={e=>setForm({...form,vacation_days:+e.target.value||0})} style={IS}/></div>
              <div><div style={{fontSize:11,fontWeight:600,color:DS.text.muted,marginBottom:4}}>Country</div><select value={form.holiday_country} onChange={e=>setForm({...form,holiday_country:e.target.value})} style={{...IS,cursor:"pointer"}}>{HOLIDAY_COUNTRIES.map(c=><option key={c}>{c}</option>)}</select></div>
            </div>
            <textarea value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})} placeholder="e.g. 'Discussed referral bonus structure & onboarding timeline.'" rows={2} style={{...IS,resize:"none"}}/>
          </div>
          {(()=>{const m=calcMargin(form);const pct=m.pct;return(<>
            <div style={{marginTop:16,padding:"14px",borderRadius:DS.radius.lg,background:pct>=40?"#f0fdf4":pct>=20?"#FEFCE8":"#fef2f2",border:`1px solid ${pct>=40?"#bbf7d0":pct>=20?"#FDE68A":"#fecaca"}`}}>
              <div style={{display:"flex",justifyContent:"space-between"}}>
                <span style={{fontSize:12,fontWeight:700,color:pct>=40?"#059669":pct>=20?"#d97706":"#dc2626"}}>{pct>=40?"💰 Viable":pct>=20?"⚠️ Moderate":"🔴 Not viable"}</span>
                <span style={{fontSize:14,fontWeight:800,color:pct>=40?"#059669":pct>=20?"#d97706":"#dc2626"}}>{pct}%</span>
              </div>
              <div style={{fontSize:11,color:DS.text.muted,marginTop:4}}>Revenue: {fmt(m.revenue)} · Cost: {fmt(m.cost)} · Margin: {fmt(m.margin)}</div>
            </div>
            <button type="button" onClick={savePricing} disabled={saving} style={{width:"100%",marginTop:12,background:`linear-gradient(135deg,#0C4A6E,#0369A1)`,color:"#fff",border:"none",borderRadius:DS.radius.md,padding:13,fontSize:14,fontWeight:600,cursor:"pointer",fontFamily:DS.font.heading,boxShadow:"0 4px 12px rgba(3,105,161,0.2)"}}>{saving?"Saving...":"💾 Save Pricing"}</button>
          </>)})()}
        </div>}
      </div>
    </div>
